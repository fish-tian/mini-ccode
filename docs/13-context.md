...
这不是 生产级系统 完整的 MicroCompact。生产级系统 可以结合缓存和完整会话记录删除旧工具结果正文；mini-ccode 第一版没有完整原始记录，所以不能把正文完全清空，只做首尾保留式压缩。
...
| 层次 | 完整产品级做法 | mini-ccode 当前实现 | 后续需要什么 |
|---|---|---|---|
...
mini-ccode 当前版本的目标是把最小可用链路做通：用户能触发、Agent 能自动调用、CLI 能看见、测试能证明不会拆坏工具协议。它不假装已经具备 生产级系统 的完整上下文工程。
