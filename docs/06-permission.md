...
| `ask` | 需要确认 | 当前版本不弹窗，返回 `permission_denied` |

当前版本把 `ask` 映射成拒绝结果，是因为 CLI 还没有逐次审批界面（permission prompt）。类型上保留 `ask`，是为了后续能自然接入交互确认。CLI 现已支持用户在启动时选择只读或全部允许模式，但这不等于逐次询问。
...
flowchart TD
  ...
  D -->|deny / ask| F["permission_denied"]
...
| `ask` 处理 | React/Ink permission prompt | 暂时返回 `permission_denied` |
...
| `tests/tool-system.test.ts` | `executeToolCall()` 权限门控和 fail-closed |
...
当前版本把 `ask` 映射为 `permission_denied`，只是因为还没有交互层。语义上 `ask` 是“需要确认”，不是永久拒绝。
